...
(() => {
  ...
  images.forEach(async (image) => {
    ...
    try {
      ...
      const setImageForDay = () => {
        const dayOfMonth = new Date().getDate();
        const index = (dayOfMonth - 1) % groups.length;
...
(() => {
  ...
  (() => {
    ...
    let playerBackgroundMode = localStorage.getItem('vcard-background-mode') || ({
      h: 'graph1',
      v: 'graph2',
    })[localStorage.getItem('vcard-visualization')] || 'graph2';
    ...
    const bindAutoAdvance = (audioEl, currentButton) => {
      ...
      audioEl.onended = () => {
        ...
        autoAdvanceTimer = window.setTimeout(() => {
          ...
          openRelativeSong(1, { play: true, fromEnded: true });
    ...
    const openSongButton = (button, options = {}) => {
      ...
      if (!options.fromEnded) cancelAutoAdvance();
    ...
    (() => {
      ...
      const updateDialogTop = () => {
        ...
        const top = playerBottom > 0
          ? Math.min(window.innerHeight, Math.round(playerBottom))
          : 0;
      ...
      const readState = () => {
        const brightnessDefault = Number.parseInt(
          vcardCssDefault('visualization-brightness', '1'),
          10
        ) - 1;
        const storedBrightness = Number.parseInt(
        ...
        return {
          background: normalizeBackground(
            ...
            || vcardStoredSetting('vcard-visualization', 'visualization', 'vertical')
          ),
  ...
  (() => {
    ...
    let backgroundMode = migrateMode(localStorage.getItem(STORAGE_KEY)) || migrateMode(localStorage.getItem('vcard-visualization')) || 'graph2';
    if (!modes.includes(backgroundMode)) backgroundMode = 'graph2';
